...
These are five tools you can easily set up and use in order audit the security of your servers and web applications just like a malicious actor would do it. Note that the examples for each tool are executed against a vulnerable-on-purpose [Hack the Box](https://www.hackthebox.eu/) Windows machine. The exact same tests can be executed against Linux servers.
