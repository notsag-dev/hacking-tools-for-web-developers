...
As you can see, nmap retrieves information about services running on a certain host. The `-A` flag means the scan is aggressive and includes OS detection, service versions detection and runs a default set of scripts. For less intrusive attacks, use `--script=safe` instead. For vulnerability detection (very useful, but more intrusive) use `--script=vuln`.
